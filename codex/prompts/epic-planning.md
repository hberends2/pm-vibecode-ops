---
description: Transform PRD and business requirements into capability-focused epics that provide engineering teams with clear functional requirements and business context without prescribing technical implementation.
...
Transform the Product Requirements Document into comprehensive epics in your ticketing system (Linear, Jira, or similar).
...
Use your ticketing system API to query all existing epics:
```
filter: "type:epic" or equivalent
limit: 250
```
...
```
Found [N] existing epics in your ticketing system. How should I proceed?
...
## Step 1: Setup Ticketing System Environment

1. Identify the target team
2. Check for existing related projects
3. Query for existing epics to avoid duplicates
...
If discovery report provided, incorporate technical landscape context.
...
## Step 4: Create Project Structure (if 4+ epics)

If 4+ capabilities identified:

1. **Create project** in your ticketing system:
   ```
...
### 5.2 Update Task List with Epic Creation Tasks
For each epic marked as CREATE in the execution plan:
```
- [ ] Create epic: [capability name]
```

## Step 6: Create Each Epic in Ticketing System (Atomic with Tracking)
...
### 6.2 Create Epic
Create an issue/epic in your ticketing system with:
...
After EACH epic creation, verify no duplicates were created by querying for the title.
...
2. Update epics to:
   - Set blockedBy relationships for epics with dependencies
   - Link related epics
...
1. **Get existing project description** from your ticketing system
...
- **Merge**: Update older epic with content from newer one
...
Run final checks:
1. **Verify all epics created** by querying project epics

2. **Verify milestones exist** by checking project milestones

3. **Verify project description contains report** by viewing the project
...
4. Technical planning decomposes epics into implementation tickets
